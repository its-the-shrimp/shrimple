...
Use `$VAR` to access a Lua variable, and `$(code)` to evaluate arbitrary Lua code and paste its return value
```html
$(
  SHRIMP = "shrimp.png"
  SHRIMP_SIZE = 100
...
- No need to specify the element name in the closing tag
(`<title></>` possible instead of `<title></title>`), though specifying it is allowed.
